...
v =  dc[:,:Variable_Cost] * 13.5 / 5;   # cost per pallet capacity ($/pallet)
...
@objective(model0, Min,
    #trans_cost*sum(demand_1to104[c]*sum(distances[i,c]*x[i,c] for i=1:num_dc) for c=1:num_counties) # Transcost
    fixed_cost_DC * sum(z[i] for i=4:num_dc)  # Fixed construction cost
    + sum(z[i] * v[i] * cc[i] for i=4:num_dc)
    );  # Variable cost
...
@constraint(model0, [i in 4:num_dc], cc[i] <= dc_cap_max[i]*z[i]);
...
# Re run new model with Z fixed
model1 = Model(solver=GurobiSolver(MIPGap=0.0001));

# The allocation decision of of DCs to counties
# Note: This decision variable determines whether DC i (row) serves county j (column)
# NOte: This decision variable is binary, because we assumed that all the demand from one county is served by the same DC
@variable(model1, x[1:num_dc,1:num_counties], Bin); #if DC i serves county j
@variable(model1, cc[1:num_dc] >= 0); # actually built DC size   sqrfoot
#@variable(model1, z[1:num_dc], Bin); #bool if DC i is built
...
# Top 3 DCs already built
@constraint(model1, [i in 1:3], z[i] == 1);
@constraint(model1, [i in 1:3], cc[i] == dc_cap_max[i]);

# Capacity can't be bigger than max cap
@constraint(model1, [i in 4:num_dc], cc[i] <= dc_cap_max[i]*z[i]);

# Each county should use one and only one DC
@constraint(model1, nosplit[c=1:num_counties], sum(x[i,c] for i=1:num_dc) == 1);

# Keep peak 8-week period inventory below DC capacityC
@constraint(model1, dc_capacity[i=1:num_dc], sum(demand_97to104[c]*x[i,c] for c=1:num_counties) <= cc[i]);
